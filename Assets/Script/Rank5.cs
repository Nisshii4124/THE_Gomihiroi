...
{
    const string RANKING_PREF_KEY5 = "ranking5";            // セーブキー
    const int RANKING_NUM5         = 5;                    // セーブデータ数
    private int[] ranking5         = new int[RANKING_NUM5]; // ランキングデータ保存

    // 使いまわしするベースのテキストオブジェクト
    public GameObject baseText;

    // ランキング表示用ゲームオブジェクト
    GameObject[] rankLabel        = new GameObject[RANKING_NUM5];
    ...
    private void Start()
    {
        ...
        // スコアをランキングに保存
        saveRanking5(TimeandScore.score);
        ...
        for (int i = 0; i < RANKING_NUM5; i++)
        {
            ...
            rankLabel[i].GetComponent<Text>().text = string.Format("{0}.  {1:00000}", i + 1, ranking5[i]);
        }
    ...
    private void Update()
    {
        ...
        {
            PlayerPrefs.DeleteKey(RANKING_PREF_KEY5);
        }
    }

    //ランキングを PlayerPrefs から取得して ranking に格納
    private  void loadRanking5()
    {
        // セーブデータをロードし、_rankingへ保存
        // セーブデータがない場合はdefaultValue = ""が_rankingへ保存される
        string _ranking = PlayerPrefs.GetString(RANKING_PREF_KEY5);
        ...
        if (_ranking.Length > 0)
        {
            ...
            for (int i = 0; i < RANKING_NUM5; i++)
            {
                ranking5[i] = int.Parse(_score[i]);
                Debug.Log(string.Format("{0}", ranking5[i]));
            }
        }
        else
        {
    ...
    // 新たにスコアを保存する
    private void saveRanking5(int new_score5)
    {
        if (ranking5[0] != 0)
        {
            ...
            for (int i = 0; i < RANKING_NUM5; i++)
            {
                if (ranking5[i] < new_score5)
                {
                    int _tmp5 = ranking5[i];
                    ranking5[i] = new_score5;
                    new_score5 = _tmp5;
                }
            }
        }
        else
        {
            ...
            ranking5[0] = new_score5;
        }

        // 「整数(int)」を「文字列(string)」に変換
        string[] str5 = new string[RANKING_NUM5];
        for (int i = 0; i < RANKING_NUM5; i++)
        {
            str5[i] = ranking5[i].ToString();
        }

        // 変換した文字列をカンマ区切りで連結し、１つの文字列へ
        string ranking_string5 = string.Join(",", str5);
        Debug.Log(string.Format("savedata5 : {0}", ranking_string5));

        // 一つにした文字列をセーブデーターとして保存
        PlayerPrefs.SetString(RANKING_PREF_KEY5, ranking_string5);
    }
